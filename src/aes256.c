 ...
 *  rcon[10] for AES-128 (as 11 round keys are needed), up to rcon[8] for
 *  AES-192, up to rcon[7] for AES-256. rcon[0] is not used in AES
 *  algorithm." */
static const u8 Rcon[11] = {
	0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

#define	AES256GetSBoxValue(num) (sbox[(num)])

/* This function produces Nb(Nr+1) round keys. The round keys are used in
 * each round to decrypt the states. */
static void KeyExpansion(u8* round_key, const u8* Key)
{
	unsigned int i, j, k;
	...
	for(i = Nk; i < Nb * (Nr + 1); i++) {
		...
		if(i % Nk == 0) {
			 ...
			 * input word and applies the S-box to each of the
			 * four bytes to produce an output word.
			 */

			/* Function Subword() */
			tempa[0] = AES256GetSBoxValue(tempa[0]);
			tempa[1] = AES256GetSBoxValue(tempa[1]);
			tempa[2] = AES256GetSBoxValue(tempa[2]);
			tempa[3] = AES256GetSBoxValue(tempa[3]);
...
static void AddRoundKey(u8 round, state_t* state, const u8* round_key)
{
	for(unsigned int i = 0; i < 4; i++) {
		for(unsigned int j = 0; j < 4; j++) {
			(*state)[i][j] ^=
				round_key[(round * Nb * 4) + (i * Nb) + j];
		}
	}
}
...
static void SubBytes(state_t* state)
{
	for(unsigned int i = 0; i < 4; i++) {
		for(unsigned int j = 0; j < 4; j++) {
			(*state)[j][i] = AES256GetSBoxValue((*state)[j][i]);
		}
	}
}
 ...
 * Note: The last call to xtime() is unneeded, but often ends up generating
 *       a smaller binary. The compiler seems to be able to vectorize the
 *       operation better this way. */
static inline u8 AES256Multiply(u8 x, u8 y)
{
	return (((y & 1) * x) ^
	   ((y >> 1 & 1) * xtime(x)) ^
	   ((y >> 2 & 1) * xtime(xtime(x))) ^
...
#define	AES256GetSBoxInvert(num) (rsbox[(num)])
...
static void InvMixColumns(state_t* state)
{
	...
	for(unsigned int i = 0; i < 4; i++) {
		...
		(*state)[i][0] = AES256Multiply(a, 0x0E) ^ AES256Multiply(b, 0x0B) ^ AES256Multiply(c, 0x0D) ^ AES256Multiply(d, 0x09);
		(*state)[i][1] = AES256Multiply(a, 0x09) ^ AES256Multiply(b, 0x0E) ^ AES256Multiply(c, 0x0B) ^ AES256Multiply(d, 0x0D);
		(*state)[i][2] = AES256Multiply(a, 0x0D) ^ AES256Multiply(b, 0x09) ^ AES256Multiply(c, 0x0E) ^ AES256Multiply(d, 0x0B);
		(*state)[i][3] = AES256Multiply(a, 0x0B) ^ AES256Multiply(b, 0x0D) ^ AES256Multiply(c, 0x09) ^ AES256Multiply(d, 0x0E);
	}
}
...
static void InvSubBytes(state_t* state)
{
	for(unsigned int i = 0; i < 4; i++) {
		for(unsigned int j = 0; j < 4; j++) {
			(*state)[j][i] = AES256GetSBoxInvert((*state)[j][i]);
		}
	}
}
...
static void Cipher(state_t* state, const u8* round_key)
{
...
static void InvCipher(state_t* state, const u8* round_key)
{
	unsigned int round = 0;
...
{
	/* The next function call encrypts the PlainText with the Key
	 * using AES algorithm. */
...
{
	/* The next function call decrypts the PlainText with the Key
	 * using AES algorithm. */
...
{
	u8 next_iv[AES_BLOCKLEN];
	for(size_t i = 0; i < length; i += AES_BLOCKLEN) {
		memcpy(next_iv, buf, AES_BLOCKLEN);
		InvCipher((state_t*) buf, ctx->round_key);
		XorWithIv((u8*) buf, ctx->iv);
